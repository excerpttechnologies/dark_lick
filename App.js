...
// import {
//   Image, StyleSheet, Dimensions, View, Text, ScrollView, TouchableOpacity,
//   RefreshControl, TouchableWithoutFeedback, TextInput, Keyboard,
//   Pressable, AppState, Alert, Platform
// } from 'react-native';
...
//     const processedText = inputType === 'numeric'
...
//         const tapPosition = Math.round((locationX / textWidth) * internalValue.length);
//         const cursorPos = Math.max(0, Math.min(internalValue.length, tapPosition));
...
//   return (
//     <View>
//       <Pressable onPress={handlePress} style={[style, styles.customInputContainer]}>
//         <View style={styles.displayInput}>
//           {isEmpty && !isFocused && (
//             <Text style={[{ color: placeholderTextColor }, styles.customPlaceholder]}>
//               {placeholder}
//             </Text>
//           )}
...
//               <Text style={styles.customInputText}>{internalValue.substring(0, selection.start)}</Text>
//               <Text style={[styles.cursor, { opacity: visible ? 1 : 0, width: 2 }]}>|</Text>
//               <Text style={styles.customInputText}>{internalValue.substring(selection.start)}</Text>
//             </View>
//           )}
//         </View>
//       </Pressable>
...
//       <TextInput
//         ref={hiddenInputRef}
//         style={[styles.hiddenInput, { position: 'absolute', width: '100%', height: 40, opacity: 0 }]}
//         value={internalValue}
...
//       const moveDistance = Math.abs(e.nativeEvent.pageY - scrollStartY.current);
...
//   const [editedData, setEditedData] = useState({ name: '', role: '', description: '' });
...
//   // NEW: 1-second silent refresh interval ref
//   const fastRefreshIntervalRef = useRef(null);
//   const [mainScrollY, setMainScrollY] = useState(0);
//   const [mainContentHeight, setMainContentHeight] = useState(0);
//   const [mainScrollViewHeight, setMainScrollViewHeight] = useState(height - 300);
//   const isInitializingDB = useRef(false);
//   const isFetchingRef = useRef(false);
//   const dbRef = useRef(null);

//   // ─── Google Auth via expo-auth-session (Expo Go compatible) ─────────────────
//  // ─── Google Auth via expo-auth-session (Expo Go compatible) ─────────────────
// const [request, response, promptAsync] = Google.useAuthRequest({
//   androidClientId: '1455597817-igff7e1mvruckv5buhii510bfn0mgro3.apps.googleusercontent.com',
//   webClientId: '1455597817-icmm1q6c810pv66fa0tmhpfb8og3lmnp.apps.googleusercontent.com',
//   scopes: ['profile', 'email'],
//   redirectUri: Platform.OS === 'web'
//     ? 'http://localhost:8081'
//     : 'com.aravinda123.myapp:/oauthredirect',
// });
...
//   // When Google auth completes successfully, fetch user info
//   useEffect(() => {
...
//   try {
//     const result = await promptAsync(); // ✅ remove useProxy: false
...
//       setUser(userData);
//       await AsyncStorage.setItem(USER_STORAGE_KEY, JSON.stringify(userData));
//       console.log('User signed in and saved:', userData.email);
//       return userData;
//     }
//     return null;
//   } catch (error) {
...
//   const syncFromServer = useCallback(async (database) => {
//     if (!database) return;

//     try {
//       console.log('Initial server sync...');
//       const response = await fetchWithTimeout(`${BACKEND_URL}/api/listproducts`, {}, 20000);
...
//         await database.runAsync(`DELETE FROM products WHERE isPending = 0 OR isPending IS NULL`);

//         for (let i = 0; i < allProducts.length; i++) {
//           const p = allProducts[i];
//           const id = typeof p._id === 'object' && p._id.$oid ? p._id.$oid : p._id;

//           if (!database) break;

//           await database.runAsync(
//             `INSERT OR IGNORE INTO products (_id, name, role, description, email, isPending, sort_order) VALUES (?, ?, ?, ?, ?, ?, ?)`,
//             id, p.name, p.role || '', p.description || '', p.email || '', 0, i
//           );
//         }
...
//       ORDER BY
//       CASE
//         WHEN isPending = 1 THEN 0
//         ELSE 1
//       END,
//       CASE
//         WHEN isPending = 0 OR isPending IS NULL THEN sort_order
//         ELSE rowid
//       END ASC
//     `);
...
//         ...p,
//         isPending: Boolean(p.isPending)
//       }));

//       setProducts(processedProducts);
//       console.log(`Loaded ${processedProducts.length} products (${processedProducts.filter(p => p.isPending).length} pending)`);
...
//         console.log('Component unmounted, aborting database initialization');
//         return null;
//       }
...
//         productIdPlaceholder TEXT,
//         rolePlaceholder TEXT,
//         descPlaceholder TEXT,
//         adminEmail TEXT,
//         updated_at DATETIME DEFAULT CURRENT_TIMESTAMP
//       );
//     `);

//       const placeholderCount = await database.getFirstAsync('SELECT COUNT(*) as count FROM placeholders');
...
//           1, 'Product Id', 'Your role in transport ?', 'Transport description', ''
//         );
//       }

//       try {
//         const tableInfo = await database.getAllAsync(`PRAGMA table_info(products)`);
//         const hasPendingColumn = tableInfo.some((column) => column.name === 'isPending');
//         const hasSortOrderColumn = tableInfo.some((column) => column.name === 'sort_order');

//         if (!hasPendingColumn) {
//           console.log('Adding isPending column...');
//           await database.execAsync(`ALTER TABLE products ADD COLUMN isPending INTEGER DEFAULT 0;`);
//         }

//         if (!hasSortOrderColumn) {
//           console.log('Adding sort_order column...');
//           await database.execAsync(`ALTER TABLE products ADD COLUMN sort_order INTEGER DEFAULT 0;`);
//         }
//       } catch (error) {
...
//       if (mountedRef.current) {
//         await loadProductsFromDB(database);
//       }
...
//     if (!dbRef.current || !isDbReady) {
//       console.log('Database not ready, skipping fetch');
//       return;
//     }
...
//     try {
//       const response = await fetchWithTimeout(`${BACKEND_URL}/api/listproducts`, {
//         headers: {
...
//       if (!currentDb || !mountedRef.current || !isDbReady) {
//         console.log('Database unavailable or component unmounted');
//         return;
//       }

//       await currentDb.runAsync(`DELETE FROM products WHERE isPending = 0 OR isPending IS NULL`);
...
//           id, product.name, product.role || '', product.description || '', product.email || '', 0, i
//         );
//       }

//       if (currentDb && mountedRef.current) {
//         await loadProductsFromDB(currentDb);
//         console.log('Database updated with server ordering preserved');
...
//         tempId, action, JSON.stringify(data), id || '', Date.now()
//       );
//       console.log('Added to offline queue:', tempId);
//       return tempId;
//     } catch (error) {
...
//     try {
//       const offlineItems = await dbRef.current.getAllAsync('SELECT * FROM offline_queue ORDER BY timestamp ASC');

//       if (offlineItems.length === 0) return;

//       console.log(`Syncing ${offlineItems.length} offline items`);

//       for (const item of offlineItems) {
//         try {
//           const itemData = JSON.parse(item.data);
//           console.log(`Syncing ${item.action}:`, item.tempId);

//           if (item.action === 'add') {
//             const response = await axios.post(`${BACKEND_URL}/api/products`, itemData);

//             if (response.status === 200 || response.status === 201) {
//               const serverProduct = response.data;
//               const serverId = typeof serverProduct._id === 'object' && serverProduct._id.$oid
//                 ? serverProduct._id.$oid
//                 : serverProduct._id;

//               await dbRef.current.runAsync(`DELETE FROM products WHERE _id = ?`, item.tempId);
//               await dbRef.current.runAsync(
...
//               console.log('Add synced:', item.tempId, '→', serverId);
//             }

//           } else if (item.action === 'edit') {
//             const response = await axios.put(`${BACKEND_URL}/api/products/${item.productId}`, itemData);
...
//                 item.productId
//               );
//               console.log('Edit synced:', item.productId);
//             }
//           }

//           await dbRef.current.runAsync('DELETE FROM offline_queue WHERE tempId = ?', item.tempId);
...
//   // Keep dbRef in sync with db state
//   useEffect(() => {
//     dbRef.current = db;
//   }, [db]);
...
//       [backgroundIntervalRef, heartbeatIntervalRef, visibilityIntervalRef, fastRefreshIntervalRef].forEach(ref => {
//         if (ref.current) {
...
//     const startIntervals = setTimeout(() => {
//       if (!mountedRef.current || !isDbReady) return;

//       // NEW: fast silent 1-second refresh so new data appears quickly
//       fastRefreshIntervalRef.current = setInterval(() => {
//         if (network.isConnected && dbRef.current && isDbReady && !isFetchingRef.current) {
//           fetchDataFromServer();
//         }
//       }, 1000);

//       backgroundIntervalRef.current = setInterval(() => {
//         if (network.isConnected && dbRef.current && isDbReady) {
//           fetchDataFromServer();
//         }
//       }, 3000);
...
//           fetchWithTimeout(`${BACKEND_URL}/api/admin-email`, { method: 'HEAD' }, 5000)
//             .catch(() => { });
//         }
//       }, 30000);

//       visibilityIntervalRef.current = setInterval(() => {
//         if (network.isConnected && dbRef.current && isDbReady && appState.match(/active|inactive/)) {
//           fetchDataFromServer();
//         }
//       }, 15000);
//     }, 3000);
...
//       });
//     };
//   }, [network.isConnected, isDbReady, appState, fetchDataFromServer, BACKEND_URL]);
...
//       if (dbRef.current) {
//         try {
//           const result = await dbRef.current.getFirstAsync('SELECT descPlaceholder FROM placeholders WHERE id = 1');
...
//     }
//     try {
//       const response = await fetchWithTimeout('https://darklick.com/api/descplace', {}, 8000);
//       if (response.ok) {
...
//       }
//     } catch (error) {
//       console.error('Error fetching descplace:', error);
//       if (dbRef.current) {
//         const result = await dbRef.current.getFirstAsync('SELECT descPlaceholder FROM placeholders WHERE id = 1');
//         setPlace2(result?.descPlaceholder || 'Transport description');
//       }
...
//       if (dbRef.current) {
//         try {
//           const result = await dbRef.current.getFirstAsync('SELECT adminEmail FROM placeholders WHERE id = 1');
...
//     }
//     try {
//       const response = await fetchWithTimeout('https://darklick.com/api/admin-email', {}, 8000);
//       if (response.ok) {
//         const data = await response.json();
...
//         if (dbRef.current) {
//           try {
//             const result = await dbRef.current.getFirstAsync('SELECT productIdPlaceholder FROM placeholders WHERE id = 1');
//             setPlaceholderText(result?.productIdPlaceholder || 'Product Id');
//           } catch (error) {
//             console.error('Error fetching cached productIdPlaceholder:', error);
//           }
//         }
//         return;
//       }
...
//       try {
//         const response = await fetchWithTimeout('https://darklick.com/api/placeholder', {}, 8000);
//         if (response.ok) {
//           const data = await response.json();
...
//       if (dbRef.current) {
//         try {
//           const result = await dbRef.current.getFirstAsync('SELECT COUNT(*) as count FROM offline_queue');
...
//     if (!network.isConnected) {
//       console.log('Adding product offline...');
//       const tempId = await addToOfflineQueue('add', productData);

//       if (tempId && dbRef.current) {
//         await dbRef.current.runAsync(
//           `INSERT INTO products (_id, name, role, description, email, isPending) VALUES (?, ?, ?, ?, ?, ?)`,
//           tempId, productData.name, productData.role, productData.description, productData.email, 1
//         );
...
//     } else {
//       try {
//         console.log('Adding product online...');
...
//     setDescription('');
//     setIsSubmitting(false);
//   }, [query, description, user, network.isConnected, isSubmitting, addToOfflineQueue, loadProductsFromDB, handleLogin, fetchDataFromServer, BACKEND_URL]);

//   const onUpdate = useCallback(async (updatedProduct) => {
//     const editData = {
...
//     if (!network.isConnected) {
//       console.log('Editing product offline...');
//       await addToOfflineQueue('edit', editData, updatedProduct._id);

//       if (dbRef.current) {
//         await dbRef.current.runAsync(
//           `UPDATE products SET name = ?, role = ?, description = ?, isPending = ? WHERE _id = ?`,
//           updatedProduct.name, updatedProduct.role, updatedProduct.description, 1, updatedProduct._id
//         );
...
//     try {
//       console.log('Editing product online...');
//       const response = await axios.put(`${BACKEND_URL}/api/products/${updatedProduct._id}`, editData);
...
//           await dbRef.current.runAsync(
//             `UPDATE products SET name = ?, role = ?, description = ?, isPending = ? WHERE _id = ?`,
//             updatedProduct.name, updatedProduct.role, updatedProduct.description, 0, updatedProduct._id
//           );
...
//     }
//   }, [network.isConnected, addToOfflineQueue, loadProductsFromDB, fetchDataFromServer, BACKEND_URL]);
...
//     setRefreshing(false);
//   }, [network.isConnected, fetchDataFromServer, syncOfflineData, GetPlace2, GetAdminEmail, initializeDB]);

//   const handleQueryChange = (text) => {
//     setQuery(text);
//   };

//   const handleProductNamePress = useCallback((product) => {
...
//   const handleMainScrollbarDrag = useCallback((event) => {
...
//     const scrollbarHeight = Math.max((mainScrollViewHeight / mainContentHeight) * mainScrollViewHeight, 40);
...
//     const newScrollOffset = scrollRatio * maxScrollOffset;
...
//   return (
//     <View style={{ flex: 1 }}>
//       <TouchableWithoutFeedback onPress={() => { setShowEmail(false); setHighlightedProductId(null); Keyboard.dismiss(); }}>
//         <ScrollView
//           ref={scrollViewRef}
...
//           }}
//           onScroll={(e) => {
//             setMainScrollY(e.nativeEvent.contentOffset.y);
//           }}
//           scrollEventThrottle={16}
...
//           }}
//           onLayout={(e) => {
//             setMainScrollViewHeight(e.nativeEvent.layout.height);
//           }}
//         >
//           <View style={styles.container}>
//             <View style={styles.topContainer}>
//               <View style={styles.leftNavigation}>
//                 <TouchableOpacity onPress={toggleEmail} style={styles.iconButton}>
//                   <Entypo name="info" size={22} color="#888" />
//                 </TouchableOpacity>
//                 <TouchableOpacity onPress={() => { initializeDB(); setQuery(''); }} style={styles.iconButton}>
...
//                 >
//                   <MaterialIcons name="send" size={24} color={isSubmitting ? "#444" : "#888"} />
//                 </TouchableOpacity>
//               )}
//             </View>
...
//                 <View key={product._id} style={[styles.productCard, { opacity: product.isPending ? 0.7 : 1 }]}>
...
//                         </TouchableOpacity>
//                         <TouchableOpacity onPress={() => setEditingId(null)} style={styles.actionButton}>
...
//                           style={[
//                             styles.productName,
//                             highlightedProductId === product._id && styles.productNameHighlighted
//                           ]}
//                           selectable={true}
//                         >
//                           {product.name}
//                         </Text>
//                       </TouchableOpacity>
//                       <Text style={styles.productRole} selectable={true}>{product.role || ""}</Text>
//                       <View style={styles.productHeader}>
//                         <Text style={styles.productDescription} numberOfLines={0} selectable={true}>
//                           {product.description || "No description provided"}
//                         </Text>
//                         {product.email === user?.email && (
//                           <TouchableOpacity onPress={() => startEdit(product)} style={styles.editButton}>
...
//           onResponderGrant={(e) => {
//             setIsDraggingMainScrollbar(true);
//           }}
//           onResponderMove={handleMainScrollbarDrag}
//           onResponderRelease={() => {
//             setIsDraggingMainScrollbar(false);
//           }}
//         >
//           <View
//             style={[
...
//               {
//                 height: Math.max(((mainScrollViewHeight / mainContentHeight) * mainScrollViewHeight) - 250, 40),
//                 transform: [{
//                   translateY: Math.max(0, Math.min(
//                     (mainScrollY / (mainContentHeight - mainScrollViewHeight)) *
//                     (mainScrollViewHeight - Math.max((mainScrollViewHeight / mainContentHeight) * mainScrollViewHeight, 40) - 300),
//                     mainScrollViewHeight - Math.max((mainScrollViewHeight / mainContentHeight) * mainScrollViewHeight, 40) - 300
//                   ))
//                 }]
//               }
